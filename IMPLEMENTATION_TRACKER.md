...
- [ ] **3.1 L0 Orbit** — monolith mesh, instanced starfield, environment lighting (≤ 200 KB) *(needs D9)*
...
| D9 | Environment lighting within 200 KB | 3.1–3.6 | Open — **blocks 3.1.** The plan's "one low-cost HDRI per layer" does not fit: a 1k Poly Haven HDRI is 1.3–1.6 MB (measured). Options: **(a)** drei `<Lightformer>` panels rendered into the environment at runtime — 0 KB, fully art-directable per layer, suits the dark cinematic tone (recommended); **(b)** one HDRI downscaled to 256×128 and shared by every layer, ~100 KB but blurry and one mood for all; **(c)** no environment map, lights only — cheapest, but reflective and glass materials (L0 monolith, L2 panes) look flat. |
