    ...
    <Section id="skills" label="Toolkit" title="What I reach for.">
      <div className="divide-y divide-slate/60 border-y border-slate/60">
        {skills.map((s) => (
          <div
            key={s.group}
            className="grid gap-2 py-5 md:grid-cols-[220px_1fr] md:gap-10 xl:grid-cols-[280px_1fr]"
          >
            ...
            <ul className="flex flex-wrap gap-x-2 gap-y-2">
              {s.items.map((i) => (
                <li
                  key={i}
                  className="rounded border border-slate/70 px-2.5 py-1 text-[0.8rem] text-bone/90 xl:text-[0.85rem]"
                >
                  {i}
                </li>
              ))}
